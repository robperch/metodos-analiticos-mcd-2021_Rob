...
marg_tabla <- marg_tabla %>% as.matrix()
```
...
error <- function(pars){
  ...
  mean((marg_tabla - tcrossprod(u, v))^2)
...
```
- Sería buena idea escalar las variables debido a que la diversidad de unidades puede afectar la precisión del resultado. La magnitud numérica de las variables puede afectar el peso resultante que se les da.
