...
```
```{r}
## Intento usando tibbles
# v %>% 
#   crossing(v) %>% 
#   mutate(afinidad = round(1*tcrossprod(u, v), 2)) %>% 
#   spread(v, afinidad)
```

¿Cómo describirías esta matriz de datos en términos de las columnas? ¿En términos 
de los renglones?
- En términos de las columnas: obtenemos los niveles generales de cada columna.
- En términos de los renglones: 
