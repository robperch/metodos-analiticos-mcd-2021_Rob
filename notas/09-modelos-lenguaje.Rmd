...
- Notemos que hay millones de W's y millones de X's.
...
- La primera parte de la ecuación es el modelo del canal (qué tanto distorsional el canal)
- La segunda parte es la probabilidad de las frases en el lenguage.
...
```
## Rob notes
- Estamos tratando de encontrar la probabilidad.
- El orden de las palabras sí importa.
```
...
```
## Rob notes
- Aquí le problema es que le vamos a asignar 0 a frases que podrían ser viables.
- Está mal asignar 0 porque sí es posible y viable.
- También es un problema enorme para escalar el problema; puede ser imposible revisar todos los datos.
```
...
```
## Rob notes
- Queremos balancear la complejidad.
```
...
```
## Rob notes
- Notemos que en unigramas no importa el orden.
- En bigramas sí importa el orden.
```
...
```
## Rob notes
- Con estos símbolos sí podemos construir efectuvamente un modelo de probabilidad.
- Con esto podemos hacer una distribución de probabilidad para tdas las frases.
- Estamos agregando la probabilidad de que haya un final de frase.
```
...
```
## Rob notes
- El problema con este modelo es que tenemos una visión muy de corto plazo.
- No se pueden modelar muchas dependencias
```
...
```{markdown}
## Rob comments
- Todo lo que no caiga en estas posibilidades le asignamos probabilidad de 0.
    - Nuestro modelo no lo cubre.
- Una mejora que podríamos aplicar es error de errores.
```
