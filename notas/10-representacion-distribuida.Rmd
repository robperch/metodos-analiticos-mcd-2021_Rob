...
```{markdown}
## Rob comments
- El éxito de las redes neuronales no solo es conectar todo con todo, es adecuarse también a la estructura de los datos.
    - La arquitectura se adecua a problema.
- En este caso también vamos a trabajar con arquitecturas especiales para NLP.
```
...
```{markdown}
## Rob comments
- No solo queremos aprender del corpus que tenemos.
    - Antes teníamos que parchar para palabras que no habíamos visto.
- Ahora queremos aprender que ciertas palabras (aunque nunca la hayamos visto) aparecen en contextos similares.
- Necesitamos menos corpus porque aprendemos más cosas... no solo lo que vimos en el corpus.
- Las vamos a hacer no-divisibles con este approach.
```
...
```{markdown}
## Rob comments
- La representación distribuida es como un vector de números
- Si ciertas palabras comparten contexto, los números de su vector se van a parecer.
    - Es como hacer un análisis de componentes principales.
    - Queremos reducir las dimensiones de las palabras en el vocabulario (e.g. de 200,000 a 100)
```
