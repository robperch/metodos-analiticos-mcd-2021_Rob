...
```
- Aquí podemos ver la mayoría de las transacciones son de pocos artículos.
- Esta tienda podría ser como un super pequeño
...
```
- Podemos ver que hay algunos pocos artículos que ocurren mucho.
...
```
- La mayor parte de las canastas están muy dispersas.
...
- Este análisis no nos es tan útil porque todos los elementos están muy dispersos.
  - Un punto interesante es que el 2.6% los tickets son sólo de cerveza.
- Para calcular el número de canastas posibles, calcularíamos:
```{r}
2^169
```
- Podemos ver que es un número gigantesco
...
- En este caso los conjuntos frecuentes serían: {1}, {2}, {3}, {1, 2}, {2, 3}
...
- Aquí practicamente estamos diciendo que queremos ver todos los sub-conjuntos.
- Hay que considerar que este es un set de datos de juego. 
...
- Nos damos cuenta de que:
  - Si queremos hacer conteo de las subcanastas y después filtrar, va a ser super super ineficiente. No es factible.
- La pregunta es ¿Por qué queremos contar todo?
  - No necesito ver los subconjuntos que aparecen pocas veces.
  - Hay que notar que cuando modificamos nuestro soporte y lo subimos, bajó mucho el número de resultados (333)
...
- El soporte nos indica que el elemento debería aparecer al menos 30%*8 (2.5) veces en las canastas.
- Los frecuentes serían 1, 2, 3 y 8.
- Ya que tenemos esos frecuentes, contamos esos pares que están en las canastas.
  - Los resultados serían: 
    - {1, 2} -> 2 (como)
    - {1, 3}
    - {2, 3}
- Ya que analizamos los pares, pasamos a analizar las tercias. La ventaja es que aquí podemos descartar con base en los pares frecuentes.
...
- Los datos simulados pueden ayudar mucho a entender mejor como funciona un algoritmo.
- Trabajar con datos reales es compmlicado porque no sabemos bien qué está pasando.
...
- Si conocemos la probabilidad de cada canasta podemos contestar cualquier pregunta de que los artículos aparezcan o no.
...
- Empezamos con algo muy ambicioso y lo vamos simplificando mucho.
  - El embudo va así:
    1. Conjunta (todas las posibles combinaciones de todos los artículos)
    2. Marginales
    3. Cocurrencia
- Las marginales serían las probabilidades de posibles outcomes (0,0 - 1,1 - 0,1 - 1,0)
- Nos vamos a concentrar en las conjuntas (1,1)
  - Las conjuntas que tienen algún 0 se vuelven menos importantes.
  - No vemos todas las marginales.
- Este análisis no da respuestas tan fáciles.

### Modelo de artículos independientes {-}
- Este es el modelo más simple que se nos puede ocurrir.
  - Este asume independencia, por eso podemos poder multiplicar las probabilidades.
  - Este no funciona mucho por dos razones principalmente:
    - Las cosas normalmente se compran para usar de forma conjunta.
    - Hay distintos tipos de personas con distintos tipos de intenciones.
...
por
$$P(X_j=1)=p_j$$.
- Si hay 500 artíulos, habrán 500 parámetros.
...
- Aquí estamos viendo que pasaría si los artículos fueran independientes.
...
- Con esta gráfica vemos que el modelo no se comporta como los datos.
  - El gran problema es que no hay asociaciones.
...
- Vemos que hay un traslape, lo cual nos indica que hay subconjutnos que son populares no por asociación, sino solo porque se consumen mucho.
...
- El punto aquí es poder estimar cierto error para aclarar qué tan frecuente es un par.
- Es como calcular cierto error que tiene el método.
...
- Esto se va a estabilizar hacia el verdadero número de pares frecuentes que hay en el modelo.
- En este caso podemos ver que nos convendría tener por lo menos 1k transacciones suficientes.
...
- Aquí "support" es 
- "confidence" es la probablidad de que compren algo con el antecedente dado.
- "lift" es una medida adicional para medir el interés de la regla.
  - Valors cercanos a "1" no son interesantes.
  - No importa el orden en el que se pongan los artículos.
...
- El conjunto de canastas frecuentes es más un pre-análisis.
...
pars <- list(support=0.002, confidence = 0.0, target='rules', 
             ext = TRUE, minlen = 2) ## el `ext` dice que pongas medidas extendidas. `minlen` es el número de artículos
...
- Aquí podemos ver que con soportes bajos, la variabilidad es mucha.
...
- Aquí podemos ver también que con soportes bajos, la variabilidad es mucha.
...
de los modelos simples.
  ...
  - Esto nos ayuda a filtrar justo esos resiltados ruidosos.
...
- El cuantil entre el que divides ayuda a quitar la variabilidad.
- La idea es muy similar a comparar con la hipótesis nula. 
  - Queremos que podría pasar fuera de la aleatoriedad normal.
...
- Estos son datos de los que esperaríamos no obtener nada ninguna relación interesante.
...
- Gráfica izquierda
  - Tenemos en general mejores valores de soporte.
- Gráfica de derecha
  - Vemos que el lift no se degrada a pesar de usar corte de Hyperlift.
...
En este caso, podemos filtrar con soporte alto, para evitar estimaciones ruidosas (por ejemplo, soporte  mínimo de 300 canastas).
  - Es como el análisis de los peces gordos.
...
- Para sacar los ingredientes más frecuentes se puede hacer un melt.
  - Después se puede hacer un group by

### Approaching problem with naive strategy. (Single ingredients)
```{r}
...
## Read original data
d_srep <- read.csv("../datos/recetas/srep00196-s3.csv") 


## Finding all the unique regions in the data and counting them.
d_srep %>% 
  group_by(region) %>%
  summarise(n = n(), .groups = "drop") %>%
  arrange(desc(n))
d_srep_regs <- unique(d_srep$region)

  
## Cleaning dataframe for process
d_srep_cln <- d_srep %>%
  mutate(id = 1:nrow(d_srep)) %>%
  gather(ingredient_id, ingredient, a1:X.1, factor_key = TRUE) %>%
  arrange(id) %>%
  filter(ingredient != "") %>%
...
## Function to count single ingredients in data.
count_ings <- function(d_srep_cln, sel_region){
  
  ## Filtering for a specific region
  if(sel_region %in% d_srep_regs){
    d_srep_cln <- filter(d_srep_cln, region == sel_region)
  }
  print(d_srep_cln)
  
  ## Counting the total number of ingredients in the filtered table.
  n_ings <- nrow(d_srep_cln)
  n_recs <- length(unique(d_srep_cln$id))
  
  ## Counting the number of ingredients for  given region.
  d_srep_cln %>%  
    group_by(ingredient) %>%
    summarise(n = n(), .groups = "drop") %>%
    mutate(
      prop_ings = round(n/n_ings, 4),
      support = round(n/n_recs, 4),
      ) %>%
    arrange(desc(n))
  
}

#### Executing created function.
d_srep_regs
sel_region <- "LatinAmerican"
count_ings(d_srep_cln, sel_region)


```

### Approaching problem with apriori algorithm.
```{r}
...
## Function to find frequent itemsets with apriori algorithm
count_ings_ap <- function(d_srep, sel_region, support, subsize){
  
  ## Filtering for a specific region
  if(sel_region %in% d_srep_regs){
    d_srep <- filter(d_srep, region == sel_region) %>%
      select(-region)
  }
  d_srep <- as.list(data.frame(t(d_srep))) %>%
    lapply(., function(z){ z[!is.na(z) & z != ""] })
  print(head(d_srep, 2))
  
  ## Applying apriori algorithm
  pars <- list(supp = support, target="frequent itemsets")
  ap_srep <- apriori(d_srep, parameter = pars)
...
support <- 0.05
subsize <- 1
count_ings_ap(d_srep, sel_region, support, subsize)
...
## Filtering dataset and propoerly arranging it
d_srep_precln <- d_srep %>%
  filter(region == sel_region) %>%
  select(-region)

d_srep_cln <- as.list(data.frame(t(d_srep_precln))) %>% 
  lapply(., function(z){ z[!is.na(z) & z != ""] })

## Finding rules
pars <- list(supp = s, confidence = 0.20, target='rules', ext = TRUE, minlen = 2)
reglas <- apriori(d_srep_cln, parameter = pars)
reglas_tam_2 <- subset(reglas, size(reglas)==2)

## Creating table with results
df_1 <- sort(reglas_tam_2, by = 'confidence') %>% DATAFRAME

df_2 <- df_1 %>% 
  select(LHS, RHS, coverage, confidence, support) %>%
  head(100) %>%
  mutate(lhs.base = n * coverage) %>%
  mutate(conf.ee = sqrt(confidence * (1 - confidence) / lhs.base)) %>%
  mutate_if(is.numeric, ~ round(.x, 2)) 
...
## Creating graph with associations
frecs <- 
...
## Formula to automate creation of graph
rules_graph <- function(supp=0.5/sqrt(n), conf){
  ...
  ## Finding rules
  pars <- list(supp = supp, confidence = conf, target='rules', ext = TRUE, minlen = 2)
  reglas <- apriori(d_srep_cln, parameter = pars)
  reglas_tam_2 <- subset(reglas, size(reglas)==2)
  ...
  print(DT::datatable(df_2 %>% select(-lhs.base)))
...
rules_graph(supp=0.06, conf=0.4)
