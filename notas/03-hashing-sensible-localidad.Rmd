...
```{markdown}
## Rob comments
- Aquí podemos ver que, entre más bandas tengamos, más fácil será sacar candidatos.
- Está gráfica fue creada con argumentos de probabilidad, sin embargo, la distribución de las similitudes de los datos es lo que puede cambiar mucho.
```
...
```{markdown}
## Rob comments
- También podemos hacer varias combinaciones de hashes y bandas para filtrar al nivel que queremos.
```
...
```{markdown}
## Rob comments
- Aquí vemos que estamos capturando solo el 53% de los de similitud 0.5.
```
...
```{markdown}
## Rob comments
- Para elgir la estrategia adecuada, debes hacer un trade-off.
- Necesitamos un criterio para decidir lo que vamos a hacer con los datos después.
- A Uber le pueden quedar fraudes no detectados pero un método eficiente.
```
...
```{markdown}
## Rob comments
- La distancia coseno es muy útil para el análisis de texto.
- Por ejemplo, dos artículos que hablan de perros pueden tener términos muy similares pero longitudes muy diferentes (un artículo es más largo que el otro).
- A pesar de esto, la distancia coseno puede ser chica y con eso podemos encontrar las similitudes.
```
...
```{markdown}
## Rob comments
- Cuando las distancias entre dos puntos es baja la probabilidad de similitud.
- Estamos explotando de nuevo la propiedad de que la distancia Jacard es igual a la del minhash.
```
...
```{markdown}
## Rob comments
- Los hashes son el vector $v$ sobre el que proyectamos los vectores que queremos comparar.
- Si caen del mismo lado del vector que escogí, entonces concluimos que son lo mismo.
```
...
```{markdown}
## Rob comments
- Todo lo de arriba es lo mismo de la tarea, la única diferencia es el método de hashing que usamos.
- Este approach es más apropiado para buscar similitud semántica, en lugar de sintáctica.
```

## LSH para distancia euclideana.
```{markdown}
## Rob comments
- Aquí se hace un vector con muchos segmentos.
- Comparamos los segmentos en los que cae la proyección de los vectores.
- Si caen en el mismo segmento, son vectores con mucha similitud.
```
