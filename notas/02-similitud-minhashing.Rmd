...
- ¿Cómo encontrar vecinos cercanos en un conjunto de datos?
  - Un caso podría ser querer hacer un join entre dos bases de datos con campos que no se parecen tanto.
...
- De aquí podemos notar que la unión siempre es más grande que la intersección. 
- Lo importante es fijarnos en las características de la unión.
...
- A estas unidades fundamentales se les llama "tokens".
...
- Aquí debemos buscar un punto medio para la longitud de la teja.


## Representación matricial

- Esta no es una representación tan tan útil en general.
- Esta hace más sentido si la guardamos de forma "rala", porque se hacen muy grandes.
...
- Para comparar cada uno de los documentos, ignoras los renglones que tienen 0's
...
- Vamos a querer ir de esta matriz enorme a otra matriz más pequeña.
...
- Con el minhash obtenemos una firma de cada documento, esta la podemos comparar y buscar firmas similares para identificar similares.
- En lugar de guardar la tabla original, podemos guardar esta tabla de firmas que es mucho más chica.
...
- Aquí podemos ver las siguientes similitudes:
  - `doc_1` & `doc_2` -> 60%
  - `doc_1` & `doc_3` -> 0%
...
- Aquí podemos ver que los resultados con las firmas son muy parecidos a los resultados del algoritmo estricto de Jaccard.
- Podemos usar lo que conocemos de estadística para calcular el error de nuestro estimador.
$$2*\sqrt{p*(1 - p)}/\sqrt{n}$$ (o algo así)
...
- Con esto demostramos el teorema que esta reducción de dimensionalidad sí funciona.

## Cálculo de firmas por documento

- Necesitamos hacer el proceso más eficiente sin hacer todas las permutaciones y demás en memoria.
...
- El uso de esta función nos da una de las posibles permutaciones.
- La unica restricción es aplicar módulo sobre un número primo.
...
- Aquí vemos que esto no funciona porque se están repitiendo números.
...
- Algo interesante es que si elegimos un primo muy grande, los valores se van a mapear a números diferentes (fuera del rango) pero que también funcionan.
- Una estrategia es agarrar un número primo grande para que las cosas nada más no den lata.
...
```
- Aquí un punto importante es usar la función para intercambiar los números binarios.
...
wc
```
- Con este approach ya pudimos sacar cluster de similitud, en lugar de hacer una prueba muy exigente y concentrarnos en los que se parecen más.
...
- Dependiendo del tipo de similitud que queramos capturar, decidimos el número de hashes.
  - Si queremos casi idénticos, haríamos solo una cubeta con muchas firmas.